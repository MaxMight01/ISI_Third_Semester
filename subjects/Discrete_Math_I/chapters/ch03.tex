...
\end{theorem}
\begin{proof}
    We will show a bijection between the set of labelled trees on $[n]$ and $[n]^{n-2}$, the set of all sequences of length $n-2$ with entries from $[n]$.

    For a tree $T$, generate a sequence of labelled trees $T_{1},\ldots,T_{n-1}$ inductively as follows: $T_{1} = T$, and $T_{2}$ is a labelled tree on $n-1$ vertices obtained by removing the leaf with the smallest label from $T_{1}$ and its incident edge. Continue this process until $T_{n-1}$, which is a tree on 2 vertices. Thus, each $T_{i}$ is a labelled tree on $n-i+1$ vertices. Denote by $x_{i}$ the leaf of $T_{i-1}$ with the smallest label; the leaf to be removed. Denote by $y_{i-1}$ the neighbour of $x_{i}$ in $T_{i-1}$. Then, the mapping $T \mapsto (y_{1},y_{2},\ldots,y_{n-2})$ gives a sequence of length $n-2$ with entries from $[n]$. This is called the \eax{Prüfer code} of the tree $T$. The claim that this mapping $T \mapsto (y_{1},y_{2},\ldots,y_{n-2})$ is a bijection is left as an exercise to the reader.
\end{proof}
