...
\begin{definition}
    ...
    \begin{align}
        \{e\} = G_{n} \nsbg G_{n-1} \nsbg \cdots \nsbg G_{2} \nsbg G_{1} \nsbg G_{0} = G
    \end{align}
    is called a \eax{normal series} for $G$ if $G_{i+1} \nsbg G_{i}$ for all $0 \leq i \leq n-1$. A normal series is called a \eax{composition series} if each quotient $G_{i}/G_{i+1}$ is simple for all $0 \leq i \leq n-1$. Moreover, $G_{i}/G_{i+1}$ are called \eax{composition factors} of $G$.
\end{definition}
...
\begin{definition}
    A group $G$ is termed a \eax{solvable group} if all its composition factors are abelian.
\end{definition}
...
We now prove the above proposition, which says that a finite group always has a composition series.
\begin{proof}
