...
Denote $\dot{\vec{r}}_{i}$ by $\vec{v}_{i}$, which can be written as
\begin{align}
...
\begin{example}
    Let us discuss \eax{Atwood's machine}, which is simply two masses $M_{1}$ and $M_{2}$ on opposite sides of a weightless pulley. If we assume that total length of the string to be $L$, then $M_{1}$ hangs $x$ length below the pulley, and $M_{2}$ hands $L-x$ length below the pulley. In this case we have
    \begin{align}
